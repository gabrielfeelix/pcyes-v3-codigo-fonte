...
function SheetOverlay({
  ...
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        /* z acima da navbar/announcement (z-50/60) e dos menus (z-70/80/90) —
           senão o topo do sheet fica coberto e o X não recebe clique. */
...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <SheetPrimitive.Close className="ring-offset-background focus:ring-ring absolute top-3.5 right-3.5 z-10 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border border-foreground/10 bg-background/80 text-foreground/70 backdrop-blur transition-colors hover:bg-foreground/10 hover:text-foreground focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none">
